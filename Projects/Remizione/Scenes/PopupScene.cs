...
namespace Remizione
{
    ...
    public sealed partial class PopupScene : Scene
    {
            ...
            : base(session.Game)
        {
            ...
            this.titleSprite = new TextSprite(session.Game, Fonts.Regular) { Color = ColorPalette.PopupTitle, PivotOrigin = RectanglePoint.Top, Position = container.BoundingBox.GetPoint(RectanglePoint.Top, 0, 12), Scale = ScaleInfo.PopupTitle, Text = title };
            this.text = new TextSprite(session.Game, Fonts.Regular) { Color = ColorPalette.TextWhite, MaximumWidth = 220, PivotOrigin = RectanglePoint.Top, Position = titleSprite.BoundingBox.GetPoint(RectanglePoint.Bottom), Scale = ScaleInfo.PopupText, Text = text };
