...
{
    // MetaItemCommand
    // Arguments: {Name} {Category} [#damage:DiceRoll] [#faith:Integer] [#knockback:Vector2] [#maximum:Integer] [#modifier:Stat] [#range:Integer] [#sound:Name] [#spirit:Integer]
    internal sealed class MetaItemCommand : NonAwaitableCommand
    {
        ...
        internal MetaItemCommand(Script script, string source, StatementBody body)
            : base(script, source, body, 2, ActionArg, BonusArg, CategoryArg, DamageArg, DurabilityArg, EffectTimingArg, FaithArg, HPArg, KnockbackArg, MaximumArg, ModifierArg, RangeArg, SoundArg, UpgradeHardnessArg)
        {
            ...
            new MetaItem(name, kind, action, effectTiming, baseDamage, modifier, bonus, knockback, maximum, hp, fp, range, durability, upgradeHardness, sound);
        }
    }
}
