...
{
    // MetaItemCommand
    // Arguments: {Name} category {ItemCategory} [#damage:DiceRoll] [#faith:Integer] [#hp:Integer] [#knockback:Vector2] [#maximum:Integer] [#modifier:Stat] [#range:Integer] [#sound:Name] [#stamina:Integer]
    internal sealed class MetaItemCommand : NonAwaitableCommand
    {
        ...
        internal MetaItemCommand(Script script, string source, StatementBody body)
            : base(script, source, body, 3, ActionArg, DamageArg, DurabilityArg, FaithArg, HPArg, ImpactWordArg, KnockbackArg, MaximumArg, MaximumLevelArg, ModifierArg, RangeArg, SoundArg)
        {
            ...
            new MetaItem(name, category, damage, modifier, knockback, maximum, hp, fp, range, durability);
        }
    }
}
