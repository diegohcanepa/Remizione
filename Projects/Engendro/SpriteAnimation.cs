...
namespace Engendro
{
    ...
    public sealed class SpriteAnimation : INamedObject
    {
        ...
        {
            return AddFrame(imageName, duration, false, string.Empty, 1, string.Empty, Rectangle.Empty, string.Empty, Vector2.Zero);
        }

        // AddFrame
        public SpriteFrame AddFrame(string imageName, int duration, string label)
        {
            return AddFrame(imageName, duration, false, label, 1, string.Empty, Rectangle.Empty, string.Empty, Vector2.Zero);
        }

        // AddFrame
        public SpriteFrame AddFrame(string imageName, int duration, bool isTriggerFrame, string label, float speedFactor, string soundName, Rectangle subArea, string gotoLabel, Vector2 actionPoint)
        {
            SpriteFrame result = new(this, frameList.Count, imageName, duration, isTriggerFrame, label, speedFactor, soundName, subArea, gotoLabel, actionPoint);
